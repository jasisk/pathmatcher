// TODO: Refactor `match` and `matchSync`
...
PathMatcher.prototype.matchSync = function (file) {
    function getPart(key) {
        var parts = {
            file: path.basename(file),
            path: path.dirname(file)
    ...
    for (var key, ret, i = 0; i < keys.length; i++) {
        key = keys[i];
        if (this._filters[key].length === 2) {
            throw new Error('`matchAsync` cannot handle async matchers');
        }
        ret = this._filters[key](getPart(key));
        if (!ret) {
            return false;
        }
    }
